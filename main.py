...
data = setup_model2(path="testData/",mtcnn = mtcnn,resnet = resnet)
knn =  KNN(data=data)
model = yolov5()
name = ['aon','most','first','error']


st.title("Facemask Detection and Facerecogition System")
check = st.button('Usecamera')
frame_window = st.image([])
cap = cv.VideoCapture(0)
stopCamera = st.button('Stop Using Camera')
